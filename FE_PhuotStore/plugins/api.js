export default function ({ $axios, redirect }, inject) {
  ...
  const api = {
    ...
    addCategory : (data,config) => axios.$post('api/v1/categories/add',data,config),
    getCategory : (id,config) => axios.$get(`api/v1/categories/${id}`,config),
    updateCategory : (id,data,config) => axios.$put(`api/v1/categories/update/${id}`,data,config),
    deleteCategory : (id,config) => axios.$delete(`api/v1/categories/delete/${id}`,config)
  }
  inject('api', api)
}
